...
def get_stock_price(symbol: str) -> dict:
    ...
    """
    url = f"https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol={symbol}&apikey=MFLJT470BUU7W94I"
    r = requests.get(url)
    return r.json()
...
# Make tool list
tools = [search_tool, rag_tool, calculator, get_stock_price, purchase_stock, get_current_weather]
...
def get_all_threads():
    all_threads = set()
    for chkp in checkpoint.list(None):
        all_threads.add(chkp.config['configurable']['thread_id'])
